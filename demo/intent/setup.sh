...
# extractor answer for real packages a judge names; installed into the base
# commit so the lockfile and package.json are not seen as agent changes
...
cp "$here/rate-limit.test.ts" rate-limit.test.ts
...
echo "  bun test ./rate-limit.test.ts  # pre-baked micro-test"
