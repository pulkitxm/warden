...
import { runWnpm, runWnpx, defaultDeps, type RunDeps } from "../src/main.ts";
...
test("defaultDeps: spawn returns the command's exit code, readFile reads files", () => {
  ...
  expect(defaultDeps.readFile(new URL("../package.json", import.meta.url).pathname)).toContain("@warden/cli");
