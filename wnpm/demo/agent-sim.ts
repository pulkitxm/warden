 ...
 *
 * Usage:
 *   WARDEN_REGISTRY=<mini> WARDEN_DOWNLOADS=<mini>/downloads/point/last-week \
...
console.log(`agent: per repo policy, vetting with Warden first: wnpx ${pkg} --json`);

const proc = Bun.spawnSync(["bun", "apps/cli/src/wnpx.ts", pkg, "--json"], {
  stdout: "pipe",
  stderr: "ignore",
  env: process.env,
});
...
if (verdict.verdict === "block") {
  ...
  console.log(`agent: Warden verdict = BLOCK [${verdict.categories.join(", ")}].`);
  ...
  console.log(`agent: Warden flagged \`${pkg}\` (WARN): ${verdict.summary}`);
  console.log(`agent: asking the user to confirm before proceeding.`);
} else {
  console.log(`agent: Warden cleared \`${pkg}\` (ALLOW). Proceeding with \`npx ${pkg}\`.`);
}
