...
test("wnpm help renders usage for root and doctor", async () => {
  for (const argv of [["--help"], ["help"], ["doctor", "--help"]]) {
    const { deps, err } = makeDeps();
    expect(await runWnpm(argv, deps)).toBe(0);
    expect(err.join("")).toContain("usage:");
    expect(err.join("")).toContain("--no-verify");
  }
});

test("wnpm doctor --no-verify passes verification opt out through", async () => {
  ...
  expect(await runWnpm(["doctor", "--no-verify"], deps)).toBe(0);
  expect(seen).toEqual([{ apply: true, verify: false }]);
});

test("wnpm doctor reports degraded no-audit and partial-audit exit codes", async () => {
  const degraded = makeDeps({ doctor: () => Promise.resolve(cleanReport({ skipped: 2 })) });
  expect(await runWnpm(["doctor"], degraded.deps)).toBe(30);
  expect(degraded.err.join("")).toContain("audit incomplete: 2 of 2");

  const partial = makeDeps({
    doctor: () => Promise.resolve(cleanReport({ audited: 1, skipped: 2 })),
  });
  expect(await runWnpm(["doctor"], partial.deps)).toBe(0);
  expect(partial.err.join("")).toContain("audit incomplete: 2 of 3");
});
...
test("wnpm doctor --no-apply skips apply and exits 10 when issues remain", async () => {
  const { deps, err } = makeDeps({
    doctor: (_dir, opts) =>
      Promise.resolve(
        cleanReport({
          issues: richReport().issues,
          applied: opts.apply ? false : undefined,
          recommended: "minimal",
        }),
      ),
  });
...
test("renderDoctorReport covers clean and deprecated shapes", () => {
  ...
  const incomplete = strip(renderDoctorReport(cleanReport({ audited: 1, skipped: 2 })));
  expect(incomplete).toContain("audit incomplete: 2 of 3 dependencies could not be audited");
  expect(incomplete).not.toContain("no dependency issues found");

  const incompleteWithIssues = strip(
    renderDoctorReport(
      cleanReport({
        audited: 1,
        skipped: 2,
        issues: [{ name: "old-lib", group: "dev", kind: "deprecated", summary: "old" }],
      }),
    ),
  );
  expect(incompleteWithIssues).toContain(
    "audit incomplete: 2 of 3 dependencies could not be audited",
  );

  const applyFailed = strip(
    renderDoctorReport(cleanReport({ recommended: "minimal", applied: false })),
  );
  expect(applyFailed).toContain("apply failed: package.json left unchanged");
  expect(applyFailed).not.toContain("run wnpm doctor without --no-apply");
});
