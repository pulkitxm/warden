...
import { readTar, readTgz, entryText } from "../src/tar.ts";

const tgz = makeTgz([
  {
    path: "package.json",
    content: JSON.stringify({ name: "demo", version: "1.0.0", scripts: { postinstall: "node x.js" } }),
  },
...
test("GNU long-name (typeflag L) entries resolve to the full path", () => {
  ...
  const entries = readTgz(makeTgz([{ path: `${longDir}/${longFile}`, content: "module.exports=42;" }]));
