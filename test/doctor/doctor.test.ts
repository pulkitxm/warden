...
test("doctor finds advisories, gates candidate fixes, and verifies plans", async () => {
  ...
  expect(report).toMatchObject({ audited: 3, skipped: 0 });
});
...
test("doctor reports a clean project with no plans", async () => {
  ...
  expect(report.recommended).toBeUndefined();
  expect(report).toMatchObject({ audited: 1, skipped: 0 });
});

test("doctor counts registry failures as skipped audits", async () => {
  const fs = memFs({
    [join("/mixed", "package.json")]: JSON.stringify({
  ...
  expect(report).toMatchObject({ audited: 1, skipped: 1 });
});
