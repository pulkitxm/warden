/**
 * Fixture packages for the mini-registry. Simulations of real 2025-2026 attack
 * patterns — never real live-malicious packages. Tarballs are built in-memory.
 */
...
export const FIXTURES: FixturePackage[] = [
  // Clean, popular package -> allow.
  {
    ...
    versions: {
      "1.3.0": {
        files: [pkgJson("left-pad", "1.3.0"), { path: "index.js", content: "module.exports=(s,n)=>String(s).padStart(n);" }],
  ...
  // Real compromised version on the blocklist -> instant hard block (no analysis).
  {
    name: "chalk",
  ...
  // Typosquat of lodash, near-zero downloads -> block (name attack).
  {
    name: "lodahs",
    downloads: 47,
    latest: "1.0.3",
    versions: {
      "1.0.3": {
        files: [pkgJson("lodahs", "1.0.3"), { path: "index.js", content: "module.exports=require('./real');" }],
        maintainer: { name: "newguy", email: "newguy@proton.me" },
        ageHours: 6,
      },
    },
  },
  // Hijacked-legit pattern (axios shape): clean 1.0.0 with provenance, then a
  // 1.0.1 that adds a postinstall reading process.env and POSTing to a raw IP,
  // published without provenance and from a changed email -> block on the diff.
  {
    name: "acme-http",
    downloads: 2_000_000,
    latest: "1.0.1",
    versions: {
      "1.0.0": {
        files: [pkgJson("acme-http", "1.0.0"), { path: "index.js", content: "module.exports={get(){}};" }],
