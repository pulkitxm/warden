...
Claim extraction can use an LLM when one is configured, including zero-key providers via the Claude or Codex CLI. When no provider is available, or the provider fails, the deterministic passes still run and the report is still published, with \`claims_status: "unverifiable"\`, the reason in \`notes\`, and an exit code of \`20\` only when a deterministic rule blocked — a hallucinated API call, or an import whose name is on the slopsquat list or was never published. Anything less blocking exits \`10\`, including a run whose only finding is a warn-level undeclared import. Scope creep is not assessed in that state, because it needs a claim set, and the report says so rather than reporting zero.
...
- **The corpus is 23 curated shapes, not a sample of real pull requests,** and its 11-case conforming population is too small to resolve a 5% budget. The floor of the measurement is stated on that page.
...
A failed claim extraction no longer erases the run. \`runIntentPipeline\` catches it and calls \`decide\` with an empty claim set, \`claimsStatus: "unverifiable"\`, and \`notes\` carrying the provider error plus a count of what the deterministic passes did find. Scope creep is suppressed in that state, because it cannot be judged without claims, and a note says so rather than the array reading empty. The exit code is 20 only when a deterministic rule blocked, meaning a hallucination or a dependency finding at \`level: "block"\`; a warn-level undeclared import leaves it at 10. The match pass degrades separately: \`llmPass\` catches everything and returns \`failed: true\`, which turns unmatched claims into \`partial\` and caps those claims at \`warn\`. \`warden intent diff\`, \`warden intent symbols\`, and \`warden intent bench\` need no model at all.
