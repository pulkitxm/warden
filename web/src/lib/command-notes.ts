...
export const COMMAND_NOTES: Record<string, CommandNote> = {
  ...
  plan: {
    ...
    gotchas: [
      ...
      "A manager-resolved graph carries versions, integrity, and tarball URLs but no lifecycle-hook or platform detail, so its execution surface reads as zero. Read `resolver` before reading the execution surface. The interception shim always uses the metadata walk, so it always sees new install scripts.",
