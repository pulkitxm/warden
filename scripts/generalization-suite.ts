#!/usr/bin/env bun
/**
 * Generalization pressure-test: a FRESH attack batch (fresh-attacks.ts, designed
 * after the fixes and NOT tuned to) plus real popular packages NOT used in the
...
const MAL: Case[] = [
  { spec: "fresh-hostname-exfil@1.0.0", label: "malicious", type: "hostname-env-exfil", expect: "miss?" },
  { spec: "fresh-dns-exfil@1.0.0", label: "malicious", type: "dns-exfil", expect: "miss?" },
  { spec: "fresh-runtime-exfil-hostname@1.0.0", label: "malicious", type: "runtime-hostname-exfil", expect: "miss?" },
  { spec: "fresh-indirect-eval@1.0.0", label: "malicious", type: "indirect-eval", expect: "miss?" },
  { spec: "fresh-reverse-shell-hostname@1.0.0", label: "malicious", type: "reverse-shell-hostname", expect: "miss?" },
  { spec: "fresh-proto-pollution@1.0.0", label: "malicious", type: "prototype-pollution", expect: "miss?" },
  { spec: "fresh-direct-url-dep@1.0.0", label: "malicious", type: "direct-url-dependency", expect: "miss?" },
  { spec: "fresh-packed-fetch-hostname@1.0.0", label: "malicious", type: "packed-fetch-hostname", expect: "miss?" },
  { spec: "fresh-runtime-exfil-ip@1.0.0", label: "malicious", type: "runtime-ip-exfil", expect: "catch" },
  ...
  { spec: "fresh-eval-charcode@1.0.0", label: "malicious", type: "eval-fromcharcode", expect: "catch" },
];

// Real popular packages NOT in the co-designed suite (fresh FP surface).
const BENIGN: Case[] = [
  "pino", "winston", "ioredis", "knex", "sequelize", "aws-sdk", "googleapis",
  "firebase", "@nestjs/core", "drizzle-orm", "tsx", "turbo", "husky",
  "lint-staged", "concurrently", "nodemailer", "stripe", "openai", "ethers",
  "web3",
...
async function main() {
  ...
  const results = await runPool(CASES, 3, async (c) => {
    try {
      ...
      return { ...c, verdict: "error" as const, score: 0, categories: [] as string[], err: (e as Error).message };
    }
  });
  reg.stop();

  let tp = 0, fp = 0, tn = 0, fn = 0;
  const misses: typeof results = [], falseAlarms: typeof results = [];
  for (const r of results) {
    const blocked = r.verdict === "block";
    if (r.label === "malicious") { if (blocked) tp++; else { fn++; misses.push(r); } }
    else { if (blocked) { fp++; falseAlarms.push(r); } else tn++; }
  }
  ...
  for (const r of misses) L.push(`- **${r.type}** \`${r.spec}\` -> ${r.verdict.toUpperCase()} (score ${r.score}) [${r.categories.join(",")}]`);
