...
Reads `package-lock.json` or `npm-shrinkwrap.json`. Other lockfile formats are reported as unsupported in `notes` rather than being silently treated as clean.
